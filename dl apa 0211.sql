with
    select_date as (
        ...
        from
            airpay_payment_txn_vn_db__txn_order_tab
        where 1=1
            and date(from_unixtime(valid_time-3600)) between current_date - interval '7' day and current_date - interval '1' day
    ),
    order_related as (
        select 
            sd.date,
            ...
            ,sum(case when category in ('QR') then cashback end) cashback_amount -- dùng ở đâu?
            -- ,round(sum(															
            -- (case														
            -- --when tb1.payment_channel_id in (20001, 20002, 24001, 24004, 24008,310004,310009,24009) and  tb1.item_id not in ('0', '') then (0.95)														
            -- when (payment_channel_id in (20115,30101) and amount=10000 and item_id not in ('0', '') )  then (0.98) -- viettel 10K 2%														
            -- when (payment_channel_id in (20115,30101) and amount>10000 and item_id not in ('0', '') ) then (0.97) -- other viettel 3%														
            ...
            -- (amount - currency_amount),2))												
            -- as direct_discount														
            ,round(sum(cashback),2) as cashback_amount -- dùng cột nào?
        ...
        from
            ...
            select 
                ...
                when tb1.payment_channel_id in (20001, 20002, 24001, 24004, 24008,310004,310009,24009,30024) then 'gppc'
                ...
                when tb1.payment_channel_id in (21071) then 'QR' -- hay in (21070,21071)?
            ...
            from airpay_vn.airpay_payment_txn_vn_db__txn_order_tab tb1
                left join airpay_payment_txn_stats_vn_db__order_extra_tab tb2 on tb1.order_id= tb2.order_id
            where 1=1
                ...
                and not (payment_payable_amount < 2000000000 and tb1.payment_channel_id in (21070,21071))
                and not (payment_payable_amount < 100000000 and tb1.payment_channel_id =500001) -- bên cost không có 2 đk này?
            ) a on a.date = sd.date
        group by sd.date
    ),
    login as (
    ...
    a30 as (
        ...
        from select_date sd 
             left join airpay_payment_txn_vn_db__txn_order_tab a on date(from_unixtime(a.valid_time-3600))  between date - interval '29' day and date 											                                                 
        where 1=1 
