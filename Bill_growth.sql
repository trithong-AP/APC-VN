...
(SELECT
o.merchant_id,
coalesce(count(distinct case when date_trunc('month', from_unixtime(o.complete_time - 3600)) = date '2020-08-01' then o.order_id end), 0) as bill_prevm,
coalesce(count(distinct case when date_trunc('month', from_unixtime(o.complete_time - 3600)) = date '2020-09-01' then o.order_id end), 0) as bill_lam,
greatest(
    coalesce(count(distinct case when date_trunc('month', from_unixtime(o.complete_time - 3600)) = date '2020-08-01' then o.order_id end), 0),
    coalesce(count(distinct case when date_trunc('month', from_unixtime(o.complete_time - 3600)) = date '2020-09-01' then o.order_id end), 0)) calculated_bill,
coalesce(count(distinct case when date_trunc('month', from_unixtime(o.complete_time - 3600)) = date '2020-10-01' then o.order_id end), 0) as bill_thm
...
when tb1.class = 3 then least(floor(greatest(tb1.bill_thm - tb1.calculated_bill, 0)/20)*40000, 40000)
