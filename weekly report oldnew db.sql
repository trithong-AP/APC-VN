...
select_date as (
    ...
    from
        shopee_vn.apc_dp_vn_db__order_tab
    where 1=1
...
non_chainstore as (
    ...
    order_related as (
        select
            ...
            count(case when o.category_name = 'Voucher' and o.carrier_name = 'Garena' and o.status = 'Completed' and m.m_type = 3 then o.order_id end) as gppc_chainstore_txn,
            ...
            round(sum(case when o.category_name = 'Voucher' and o.carrier_name = 'Garena' and o.status = 'Completed' and m.m_type = 3 then o.total_discount_price end)/100000/23000,0) as chainstore_gppc_gtv,
        ...
        from select_date s
            left join shopee_vn.apc_dp_vn_db__order_tab o on s.valid_date = date(from_unixtime(o.payment_time - 3600))
    ...
    new_merchant as (
        ...
        where m.merchant_source = 60000 and u.role = 1 and m.m_status = 1 and m.m_type <> 3
    ...
    a1 as (
        ...
        from select_date s
            left join shopee_vn.apc_dp_vn_db__order_tab o1 on s.valid_date = date(from_unixtime(o1.payment_time - 3600))
            left join airpay_vn.airpay_merchant_info_vn_db__merchant_info_tab m on o1.merchant_id = m.id
        where
    ...
    a7 as (
        ...
        from select_date s
            left join shopee_vn.apc_dp_vn_db__order_tab o7 on s.valid_date >= date(from_unixtime(o7.payment_time - 3600)) and s.valid_date - interval '7' day < date(from_unixtime(o7.payment_time - 3600))
            left join airpay_vn.airpay_merchant_info_vn_db__merchant_info_tab m on o7.merchant_id = m.id
        where
    ...
    a30 as (
        ...
        from select_date s
           left join shopee_vn.apc_dp_vn_db__order_tab o30 on s.valid_date >= date(from_unixtime(o30.payment_time - 3600)) and s.valid_date - interval '30' day < date(from_unixtime(o30.payment_time - 3600))
           left join airpay_vn.airpay_merchant_info_vn_db__merchant_info_tab m on o30.merchant_id = m.id
        where
    ...
    total_merchant as (
        ...
        left join shopee_vn.apc_wallet_vn_db__bank_account_tab b on b.user_id = u.uid
        where 1=1
            and m.merchant_source = 60000 and m.m_type <> 3 and m.m_status = 1 and u.role = 1
            and not regexp_like(lower(m.m_name), 'test|internal')
        group by 1 
    ),
    topup as (
        ...
        from select_date s
            ...
            left join shopee_vn.apc_account_vn_db__user_tab u on t.uid = u.uid
...
chainstore as (
    ...
    order_related as (
        ...
        where 1=1
            and (regexp_like(lower(first_name),'chainstore') or regexp_like(lower(last_name),'chainstore'))
        group by 1
    ),
    new_merchant as (
        select
...
select
    ...
    sum(fin_txn + home_txn + fe_txn + acs_ocb_txn + otherfin_txn) as fin_txn,
    sum(otherbills_txn + water_txn + elec_txn) as otherbills_txn,
    sum(telco_txn + mobi_txn + vina_txn + viettel_txn + othertelco_txn) as telco_txn,
    sum(othergames_txn) as othergames_txn,    
    sum(ViViet_txn) ViViet_txn,
    sum(otherproducts_txn) as otherproducts_txn,
    sum(gppc_gtv) as gppc_gtv,
    sum(chainstore_gppc_gtv) as chainstore_gppc_gtv,
    sum(fin_gtv + home_gtv + fe_gtv + acs_ocb_gtv + otherfin_gtv) as fin_gtv,
    sum(otherbills_gtv + water_gtv + elec_gtv) as otherbills_gtv,
    sum(telco_gtv + mobi_gtv + vina_gtv + viettel_gtv + othertelco_gtv) as telco_gtv,
    sum(othergames_gtv) as othergames_gtv,    
    sum(ViViet_gtv) ViViet_gtv,
    sum(otherproducts_gtv) as otherproducts_gtv,
    sum(new_merchants) new_merchants,
